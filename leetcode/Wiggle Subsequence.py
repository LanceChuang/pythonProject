...
class Solution(object):
    def wiggleMaxLength(self, nums):
        ...
        for i in range(1,len(nums)):
            ...
            # ignore diff of 0 as they don't count as turning points
            if diff != 0:
                
                judge.append(diff)
        
        # if there are diffs only 0 then seq == 1
        if not judge:
            return 1
        
        count = 1 # from now minimun seq is 1
        
        for x in range(1,len(judge)):
            product = judge[x] * judge[x-1]
            if product < 0: # must have positive and negative
                count += 1
        
        return count + 1

target = [1,17,5,10,13,15,10,5,16,8]
Sol = Solution()
ans = Sol.wiggleMaxLength(target)
print(ans)